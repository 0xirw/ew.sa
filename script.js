...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const titleText = "マダラ";
  const prefix = "@";
  let titleIndex = 0; // Start at 0 to type from beginning
  ...
  function animateTitle() {
    // Typing effect with slower pace
    if (!isDeleting && titleIndex < titleText.length) {
      currentTitleText = titleText.slice(0, titleIndex + 1);
      document.title = prefix + currentTitleText + (titleCursorVisible ? '|' : ' ');
      titleIndex++;
      setTimeout(animateTitle, 300); // Slower typing
    } 
    // Deleting effect with slower pace (but never delete the prefix)
    else if (isDeleting && titleIndex > 0) { // Stop at index 0 to delete all text but keep prefix
      currentTitleText = titleText.slice(0, titleIndex - 1);
      document.title = prefix + currentTitleText + (titleCursorVisible ? '|' : ' ');
      titleIndex--;
      setTimeout(animateTitle, 250); // Slower deleting
    }
    // Switch to deleting mode when fully typed with longer delay
    else if (titleIndex === titleText.length) {
      isDeleting = true;
      setTimeout(animateTitle, 2000); // Longer pause when fully typed
    }
    // Switch back to typing mode when fully deleted with delay (but keep prefix)
    else if (titleIndex === 0) { // Stop at index 0 to delete all text but keep prefix
      isDeleting = false;
      setTimeout(animateTitle, 1000); // Pause when fully deleted
    }
  }
  ...
  startScreen.addEventListener('click', () => {
    ...
    // Mute background music and play video instead
